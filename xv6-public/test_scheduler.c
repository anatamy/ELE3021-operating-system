...
test_stride(int portion)
{
    ...
    // CODE MODIFIED by hkko
    // Exception handling code added.
    // RETURN VALUE of cpu_share system call:
    // 1) -1: cpu_share failed
    // 2) 1: exception handling because it exceeds the limitation
    // 3) 0: no problem
    
    int rt = cpu_share(portion);
    
    if(rt == 1) {
        printf(1, "EXCEED 20%: exception handling\n");
    }
...
main(int argc, char *argv[])
{
	...
	struct workload workloads[WORKLOAD_NUM] = {
		...
		//{test_mlfq, MLFQ_NONE}, // added
		/* Process scheduled by MLFQ scheduler, does not yield itself */
		{test_mlfq, MLFQ_NONE},
		/* Process scheduled by default scheduler */
		{test_default, 0},
		//{test_default, 0}, // added
		//{test_default, 0}, // added
	};
